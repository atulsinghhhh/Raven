...
async function inboundStats(page) {
  return page.evaluate(async () => {
    ...
    let frames = 0, bytes = 0, inbound = 0;
    for (const pc of pcs) {
      const stats = await pc.getStats();
      stats.forEach((r) => {
        if (r.type === 'inbound-rtp') { inbound++; frames += r.framesDecoded ?? 0; bytes += r.bytesReceived ?? 0; }
      });
    }
...
for (const p of [pA, pB]) {
  ...
  p.on('console', (m) => { const t = m.text(); if (t.includes('[flutter')) console.log(`    · ${t.slice(0, 160)}`); });
}
...
try {
  ...
  await pA.goto(url(8799, { token: alice.token, endpoint: alice.endpoint, roomId: room.id, iceServers: ice, publish: 'true' }));
  const sA = await waitForState(pA, (s) => s.ready === true || s.error, { label: 'alice ready', timeout: 60000 });
  record('raven_rtc', 'join() connects to the SFU', !sA.error && sA.connectionState === 'connected', sA.error ? JSON.stringify(sA.error) : `connectionState=${sA.connectionState}`);
  record('raven_rtc', 'enableCamera() publishes', sA.cameraPublished === true, `cameraPublished=${sA.cameraPublished}`);
  record('raven_rtc', 'enableMicrophone() publishes', sA.microphonePublished === true, `microphonePublished=${sA.microphonePublished}`);

  // Issue 5: local track must be visible before enableCamera() resolves.
  const seen = sA.localCameraFirstSeenAtMs, resolved = sA.enableCameraResolvedAtMs;
  record('raven_rtc', 'local preview notified before negotiation completes (0.1.8)',
    typeof seen === 'number' && typeof resolved === 'number' && seen <= resolved,
    `firstSeen=${seen}ms resolved=${resolved}ms`);

  await new Promise((r) => setTimeout(r, 8000));

  await pB.goto(url(8799, { token: bob.token, endpoint: bob.endpoint, roomId: room.id, iceServers: ice, publish: 'true' }));
  const sB = await waitForState(pB, (s) => s.ready === true || s.error, { label: 'bob ready', timeout: 60000 });
  record('raven_rtc', 'second participant joins a live room', !sB.error && sB.connectionState === 'connected', sB.error ? JSON.stringify(sB.error) : `connectionState=${sB.connectionState}`);
  ...
  const both = (s, who) => { const v = s.remoteLiveSources?.[who] ?? []; return v.includes('camera') && v.includes('microphone'); };
  const aSeesB = await waitForState(pA, (s) => both(s, 'bob'), { label: 'alice sees bob camera+mic', timeout: 40000 }).catch((e) => ({ _err: e.message }));
  const bSeesA = await waitForState(pB, (s) => both(s, 'alice'), { label: 'bob sees alice camera+mic', timeout: 40000 }).catch((e) => ({ _err: e.message }));
  const aSrc = aSeesB._err ? [] : (aSeesB.remoteLiveSources?.bob ?? []);
  const bSrc = bSeesA._err ? [] : (bSeesA.remoteLiveSources?.alice ?? []);
  record('raven_rtc', 'first joiner sees second joiner tracks (engine)', aSrc.includes('camera') && aSrc.includes('microphone'), `alice.remoteLiveSources.bob=${JSON.stringify(aSrc)}`);
  record('raven_rtc', 'second joiner sees first joiner tracks (engine)', bSrc.includes('camera') && bSrc.includes('microphone'), `bob.remoteLiveSources.alice=${JSON.stringify(bSrc)}`);

  // ---- Real media, not just signaling: two samples several seconds apart.
  await new Promise((r) => setTimeout(r, 6000));
  const a1 = await inboundStats(pA), b1 = await inboundStats(pB);
  await new Promise((r) => setTimeout(r, 6000));
  const a2 = await inboundStats(pA), b2 = await inboundStats(pB);
  record('raven_rtc', 'alice decodes real media from bob', a2.frames > a1.frames && a2.bytes > a1.bytes, `frames ${a1.frames}->${a2.frames}, bytes ${a1.bytes}->${a2.bytes}`);
  record('raven_rtc', 'bob decodes real media from alice', b2.frames > b1.frames && b2.bytes > b1.bytes, `frames ${b1.frames}->${b2.frames}, bytes ${b1.bytes}->${b2.bytes}`);
  ...
  const gone = await waitForState(pB, (s) => !(s.remoteLiveSources ?? {}).alice, { label: 'bob sees alice depart', timeout: 30000 }).catch((e) => ({ _err: e.message }));
  record('raven_rtc', 'leave() removes the participant from the remote roster', hasLeave && !gone._err, gone._err ? gone._err.slice(0, 120) : 'bob.remoteLiveSources no longer lists alice');
} finally {
  await bA.close(); await bB.close(); server.close();
