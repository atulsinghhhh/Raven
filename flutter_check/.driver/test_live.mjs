...
try {
  ...
  console.log('COULD NOT CREATE STREAM:', e.message);
  process.exit(1);
}
...
for (const p of [pH, pV]) {
  ...
  p.on('console', (m) => {
    const t = m.text();
    if (t.includes('[flutter')) console.log(`    · ${t.slice(0, 150)}`);
  });
}
...
try {
  ...
  record(
    'raven_live',
    'host credentials carry both planes',
    !!hostCreds.rtc?.token && !!hostCreds.chat?.token,
  ...
  record(
    'raven_live',
    'RavenLiveStream.join() composes rtc + chat',
    !sH.error && sH.ready === true,
    sH.error ? JSON.stringify(sH.error) : `connectionState=${sH.connectionState}`,
  );
  record('raven_live', 'host publishes camera', sH.cameraPublished === true, `cameraPublished=${sH.cameraPublished}`);
  record(
    'raven_live',
    'host publishes microphone',
    sH.microphonePublished === true,
    `microphonePublished=${sH.microphonePublished}`,
  );
  record(
    'raven_live',
    'chat connects alongside the room',
    !sH.connectError,
    sH.connectError ?? 'no chat connect error',
  );
  ...
  record(
    'raven_live',
    'viewer role maps to VIEWER (not host)',
    viewerCreds.role === 'VIEWER',
    `role=${viewerCreds.role}`,
  );
  ...
  record(
    'raven_live',
    'viewer joins a live stream in progress',
    !sV.error && sV.ready === true,
    sV.error ? JSON.stringify(sV.error) : `connectionState=${sV.connectionState}`,
  );
  record(
    'raven_live',
    'viewer does not publish',
    sV.cameraPublished !== true && sV.microphonePublished !== true,
    `camera=${sV.cameraPublished} mic=${sV.microphonePublished}`,
  );
  ...
  record(
    'raven_live',
    'viewer decodes real media from the host',
    v2.frames > v1.frames && v2.bytes > v1.bytes,
    `frames ${v1.frames}->${v2.frames}, bytes ${v1.bytes}->${v2.bytes}`,
  );

  const ended = await post(`/v1/live-streams/${sid}/end`, {})
    .then(() => true)
    .catch((e) => e.message);
  record(
    'raven_live',
    'stream can be ended server-side',
    ended === true,
    ended === true ? 'LIVE -> ENDED' : String(ended).slice(0, 110),
  );
} finally {
  await bHost.close();
  await bView.close();
