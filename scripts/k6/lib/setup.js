...
export function provisionApiKey(baseUrl) {
  ...
  const registerRes = http.post(`${baseUrl}/v1/auth/register`, JSON.stringify({ email, password }), {
  ...
  const projectRes = http.post(`${baseUrl}/v1/projects`, JSON.stringify({ name: `k6-load-test-${Date.now()}` }), {
  ...
  const keyRes = http.post(`${baseUrl}/v1/projects/${projectId}/api-keys`, JSON.stringify({ name: 'k6-load-test' }), {
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${accessToken}` },
  });
