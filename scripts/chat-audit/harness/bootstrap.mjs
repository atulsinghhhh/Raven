...
const proj = must(await http('/v1/projects', { method: 'POST', token: jwt, body: { name: `audit-${suffix}` } }), 201, 'create project');
const key = must(await http(`/v1/projects/${proj.id}/api-keys`, { method: 'POST', token: jwt, body: { name: 'audit' } }), 201, 'create api key');

const out = { suffix, email, jwt, projectId: proj.id, apiKey: key.key, base: process.env.RAVEN_BASE ?? 'http://localhost:4177' };
