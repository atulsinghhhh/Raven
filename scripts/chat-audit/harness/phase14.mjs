...
const mk = async (user) => {
  ...
  await page.waitForFunction(() => window.__acme?.api && window.__acme.chat.connectionState === 'connected', null, { timeout: 20000 });
...
await test('Alice → Bob message delivery in a real browser', async () => {
  ...
  ok(bobMsgs.some((m) => m.text === 'hello bob, from a real browser' && m.from === 'alice'), 'bob rendered it with the right sender');
  const aliceMsgs = (await res(alice)).messages.filter((m) => m.text);
  ok(aliceMsgs.some((m) => m.text === 'hello bob, from a real browser'), 'alice sees her own message echoed');
});
...
await test('Bob → Alice reply', async () => {
  ...
  await sleep(1200);
  const aliceMsgs = (await res(alice)).messages.filter((m) => m.text);
  ok(aliceMsgs.some((m) => m.text === 'hi alice, got it' && m.from === 'bob'), 'alice received bob\'s reply');
});
...
await test('typing indicators cross the wire between the two browsers', async () => {
  ...
  ok(bobTyping.some((t) => t.userId === 'alice' && t.isTyping), 'bob saw alice start typing');
  await api(alice, 'stopTyping');
  await sleep(800);
  ok((await res(bob)).typing.some((t) => t.userId === 'alice' && !t.isTyping), 'bob saw alice stop');
...
await test('read receipts flow browser-to-browser', async () => {
  ...
  ok(aliceReads.some((r) => r.userId === 'bob'), 'alice was told bob read');
...
await test('reactions flow browser-to-browser', async () => {
  ...
  ok(bobRx.some((r) => r.kind === 'added' && r.emoji === '👍' && r.userId === 'alice'), 'bob saw the reaction');
  await api(alice, 'unreactTo', 0, '👍');
  await sleep(1000);
  ok((await res(bob)).reactions.some((r) => r.kind === 'removed' && r.emoji === '👍'), 'bob saw the removal');
});

await test('history + pagination from the browser', async () => {
  for (let i = 0; i < 12; i++) { await api(alice, 'send', `page-${i}`); }
  await sleep(1500);
...
await test('soft delete renders as a placeholder, not a hole', async () => {
  ...
  ok(bobMsgs.some((m) => m.deleted === deleted.id), 'bob got the deletion event and kept the position');
});
...
await test('typing does not create messages (browser view agrees with the DB)', async () => {
  ...
  for (let i = 0; i < 8; i++) { await api(alice, 'startTyping'); await sleep(80); }
  await api(alice, 'stopTyping');
  await sleep(800);
...
await test('Alice closes her tab → Bob sees her go offline; Alice reopens → back online', async () => {
  await alice.page.close(); await alice.context.close();
  await sleep(2000);
  ...
  await sleep(1500);
  const p2 = await api(bob, 'presence');
  ok(p2.some((x) => x.userId === 'alice' && x.status === 'online'), 'alice back online after reopening the page');
...
await test('gateway restart → both browsers reconnect with NO page refresh', async () => {
  ...
  while (Date.now() < deadline && !(await state(alice) === 'connected' && await state(bob) === 'connected')) await sleep(1000);
  ...
  await sleep(1500);
  ok((await res(bob)).messages.some((m) => m.text === 'after the gateway restart'), 'fan-out works after the restart with no refresh');
});
...
await test('no uncaught page errors in either browser for the whole run', async () => {
  ...
  eq(alice.errors.filter((e) => !ignore(e)), [], `alice page errors: ${JSON.stringify(alice.errors)}`);
  eq(bob.errors.filter((e) => !ignore(e)), [], `bob page errors: ${JSON.stringify(bob.errors)}`);
  note(`expected transport errors during the deliberate outage: alice ${alice.errors.length}, bob ${bob.errors.length}`);
});
