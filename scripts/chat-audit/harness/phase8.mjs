...
const grant = async (u, extra = {}) =>
  must(await http('/v1/chat/tokens', { method: 'POST', token: apiKey, body: { userId: u, ...extra } }), 201, u);
...
await test('non-member → send message: refused, nothing persisted', async () => {
  const r = await http(`/v1/chat/conversations/${room}/messages`, {
    method: 'POST',
    token: mg.token,
    body: { text: 'intrusion' },
  });
...
await test('non-member → add member: refused', async () => {
  const r = await http(`/v1/chat/conversations/${room}/members`, {
    method: 'POST',
    token: mg.token,
    body: { userId: 'mallory' },
  });
...
await test('non-member → open a WebSocket and join: refused without confirming the room exists', async () => {
  ...
  let code = null;
  try {
    await m.request('room.join', { room });
  } catch (e) {
    code = e.code;
  }
...
await test('non-member never receives fan-out', async () => {
  const m = new Client(mg.token, 'mallory2');
  await m.connect();
  try {
    await m.request('room.join', { room });
  } catch {
    // Expected to be refused — mallory is not a member. Either way what
    // matters is the fan-out assertion below, not how the join ended.
  }
  m.clear();
...
await test("cross-project isolation: another project's key cannot see this conversation", async () => {
  const reg = must(
    await http('/v1/auth/register', {
      method: 'POST',
      body: { email: `other-${S}@raven.local`, password: 'correct-horse-battery-staple' },
    }),
    201,
    'reg',
  );
  const p2 = must(
    await http('/v1/projects', { method: 'POST', token: reg.accessToken, body: { name: `other-${S}` } }),
    201,
    'proj',
  );
  const k2 = must(
    await http(`/v1/projects/${p2.id}/api-keys`, { method: 'POST', token: reg.accessToken, body: { name: 'k' } }),
    201,
    'key',
  );
...
await test('removed member → send message: refused', async () => {
  const bobClient = new Client(bg.token, 'bob');
  ...
  must(
    await http(`/v1/chat/conversations/${room}/members/bob`, { method: 'DELETE', token: apiKey }),
    204,
    'remove bob',
  );
  ...
  let code = null;
  try {
    await bobClient.request('message.send', { room, text: 'after removal', clientMessageId: `${S}-bob2` });
  } catch (e) {
    code = e.code;
  }
...
await test('LIVE REVOCATION: a socket joined BEFORE removal keeps receiving fan-out', async () => {
  ...
  must(
    await http(`/v1/chat/conversations/${room}/members`, { method: 'POST', token: apiKey, body: { userId: 'bob' } }),
    201,
    're-add bob',
  );
...
await test('a revoked-by-expiry token is rejected', async () => {
  const short = must(
    await http('/v1/chat/tokens', {
      method: 'POST',
      token: apiKey,
      body: { userId: 'alice', conversations: [room], ttlSeconds: 60 },
    }),
    201,
    'short',
  );
  ...
  const forged = short.token.split('.');
  forged[1] = Buffer.from(JSON.stringify({ ...claims, exp: claims.exp + 999999 })).toString('base64url');
