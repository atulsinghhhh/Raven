...
const room = (must(await http('/v1/chat/conversations', { method: 'POST', token: apiKey, body: {
  name: S, members: [{ userId: 'alice' }, { userId: 'bob' }] } }), 201, 'conv')).publicId;
const g = async (u) => must(await http('/v1/chat/tokens', { method: 'POST', token: apiKey, body: { userId: u, conversations: [room], ttlSeconds: 21600 } }), 201, u);
const ag = await g('alice'), bg = await g('bob');
...
// 1. Frame size
let frameBytes = 0, cursorBytes = 0, n = 0;
b.ws.on('message', (raw) => {
  const f = JSON.parse(raw.toString());
  if (f.type === 'message') { frameBytes += raw.length; cursorBytes += (f.message.cursor ?? '').length + '"cursor":,'.length; n++; }
});
...
console.log(`  persist p50/p95/max ${pct(persist, .5)} / ${pct(persist, .95)} / ${Math.max(...persist)} ms`);
console.log(`  mean frame          ${(frameBytes / n).toFixed(0)} bytes`);
console.log(`  of which cursor     ${(cursorBytes / n).toFixed(0)} bytes (${(100 * cursorBytes / frameBytes).toFixed(1)}%)`);
...
const timeIt = async (qs, runs = 12) => { const s = []; for (let i = 0; i < runs; i++) { const t = Date.now(); must(await http(`/v1/chat/conversations/${room}/messages?${qs}`, { token: ag.token }), 200, 'q'); s.push(Date.now() - t); } return pct(s, .5); };
...
back = must(await http(`/v1/chat/conversations/${room}/messages?limit=100&before=${encodeURIComponent(back.nextCursor)}`, { token: ag.token }), 200, 'o2');
