...
await test('1–6 read, 7–10 unread (verified against message list)', async () => {
  ...
  const read = page.data.filter((m) => Date.parse(m.createdAt) <= lastReadAt).map((m) => m.text).sort();
  const unread = page.data.filter((m) => Date.parse(m.createdAt) > lastReadAt).map((m) => m.text).sort();
...
await test('reconnect preserves read state', async () => {
  ...
  bob = new Client(bg.token, 'bob2'); await bob.connect(); await bob.request('room.join', { room });
  const st = must(await http(`/v1/chat/conversations/${room}/read-state`, { token: bg.token }), 200, 's');
...
await test('read.mark on a message in another conversation is refused', async () => {
  const other = must(await http('/v1/chat/conversations', { method: 'POST', token: apiKey, body: {
    name: S + '-other', members: [{ userId: 'alice' }] } }), 201, 'other');
  const m = must(await http(`/v1/chat/conversations/${other.publicId}/messages`, { method: 'POST', token: apiKey, body: { senderId: 'alice', text: 'secret' } }), 201, 'send');
  let code = null;
  try { await bob.request('read.mark', { messageId: m.id }); } catch (e) { code = e.code; }
