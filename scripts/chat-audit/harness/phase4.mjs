...
const conv = must(await http('/v1/chat/conversations', { method: 'POST', token: apiKey, body: {
  name: `p4-${S}`, members: [{ userId: 'alice' }, { userId: 'bob' }],
} }), 201, 'conv');
...
const presence = async () => must(await http(`/v1/chat/conversations/${room}/presence`, { token: apiKey }), 200, 'presence');
...
await test('second participant sees presence event', async () => {
  ...
  eq(joined.presence.map((p) => p.userId).sort(), ['alice', 'bob'], 'join snapshot carries everyone present, joiner included');
  await sleep(300);
...
await test('offline on clean disconnect', async () => {
  bobC.clear();
  const gone = bobC.waitFor((f) => f.type === 'presence' && f.userId === 'alice' && f.status === 'offline', 5000, 'alice offline');
  a1.close();
  await gone;
  await sleep(200);
  const p = await presence();
  eq(p.map((x) => x.userId), ['bob'], 'alice no longer present');
});
...
await test('MULTI-SESSION: Alice in 2 browsers, close browser 1 → Alice must stay online', async () => {
  ...
  const offlineEvents = bobC.frames.filter((f) => f.type === 'presence' && f.userId === 'alice' && f.status === 'offline');
