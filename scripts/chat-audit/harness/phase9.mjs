  ...
  for (let i = 0; i < n; i++) {
    ...
    acks.push(
      await client.request(
        'message.send',
        { room, text: `${prefix}-${i}`, clientMessageId: `${S}-${prefix}-${i}` },
        20000,
      ),
    );
...
await test('100 rapid sequential messages: no loss, no duplicates, ordered', async () => {
  ...
  ok(
    acks100.every((a) => a.deduplicated === false),
    'none deduplicated',
  );
  note(
    `${elapsed}ms wall clock, mean persistLatency ${Math.round(acks100.reduce((s, a) => s + a.persistLatencyMs, 0) / 100)}ms`,
  );
  ...
  eq(
    ...
    acks100.map((a) => a.message.text),
    'delivery order == send order (single sender)',
  );
});
...
await test('persistence: all 100 in Postgres in the same order', async () => {
  ...
  eq(
    seq.map((m) => m.text),
    acks100.map((a) => a.message.text),
    'stored order matches ack order',
  );
  eq(
    seq.map((m) => m.id),
    acks100.map((a) => a.message.id),
    'ids match acks exactly',
  );
});
...
await test('concurrent senders: Alice 100 + Bob 100 — no loss, no duplicates', async () => {
  ...
  for (
    let i = 0;
    i < 100 &&
    (bob.frames.filter((f) => f.type === 'message').length < 200 ||
      alice.frames.filter((f) => f.type === 'message').length < 200);
    i++
  )
    await sleep(200);
  for (const [name, c] of [
...
await test('ordering contract: per-sender order is preserved for every observer', async () => {
  ...
  note(
    'Per-sender FIFO holds. Cross-sender interleaving is NOT guaranteed to be identical for all observers, and the docs should say so — see below.',
  );
});

await test('cross-sender interleaving: observed vs createdAt order', async () => {
  const byCreatedAt = must(await http(`/v1/chat/conversations/${room}/messages?limit=100`, { token: apiKey }), 200, 'h')
    .data.filter((m) => m.text?.startsWith('c'))
    .map((m) => m.text);
  const aliceOrder = alice.frames
    .filter((f) => f.type === 'message' && f.message.text.startsWith('c'))
    .map((f) => f.message.text);
  const bobOrder = bob.frames
    .filter((f) => f.type === 'message' && f.message.text.startsWith('c'))
    .map((f) => f.message.text);
  ...
  note(
    `delivery order == createdAt order: ${JSON.stringify(aliceOrder.slice(0, 8))} vs stored ${JSON.stringify(byCreatedAt.slice(-8).reverse())}`,
  );
...
await test('idempotent retry under concurrency: 10 parallel sends of one key → 1 row', async () => {
  ...
  const results = await Promise.all(
    Array.from({ length: 10 }, () =>
      alice.request('message.send', { room, text: 'raced', clientMessageId: key }, 20000).then(
        (r) => r,
        (e) => ({ err: e.code }),
      ),
    ),
  ...
  note(
    `${okResults.length} succeeded, ${results.length - okResults.length} errored (${JSON.stringify(results.filter((r) => r.err).map((r) => r.err))})`,
  );
...
await test('rate limiting kicks in and reports retryAfterSeconds', async () => {
  note('verified separately at default CHAT_SEND_RATE_LIMIT=30/10s — this run raises the limit to isolate reliability');
  ok(true, 'see phase9-ratelimit note');
});
