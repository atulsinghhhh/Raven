...
import { test, note, eq, summary } from './runner.mjs';
...
const mkRoom = async (name, users) => {
  const c = must(
    await http('/v1/chat/conversations', {
      method: 'POST',
      token: apiKey,
      body: { name, members: users.map((u) => ({ userId: u })) },
    }),
    201,
    'conv',
  );
...
const apiRss = () => {
  try {
    return Math.round(
      Number(execSync(`ps -o rss= -p $(pgrep -f 'node dist/main.js' | head -1)`).toString().trim()) / 1024,
    );
  } catch {
    return -1;
  }
};
const settledRss = async () => {
  await sleep(2500);
  return apiRss();
};
const metrics = [];
...
await test('L1 — 100 messages, 1 sender, 1 receiver: latency + integrity', async () => {
  ...
  b.ws.on('message', (raw) => {
    ...
    if (f.type === 'message') arrival.set(f.message.id, Date.now());
  });
  ...
  eq(
    pg(`SELECT count(*) FROM chat_messages WHERE content LIKE '${S}-l1-%' OR content LIKE 'l1-%'`) >= '100',
    true,
    'persisted',
  );
  const m = {
    test: 'L1 100 msgs',
    wallMs: wall,
    throughput: +(100 / (wall / 1000)).toFixed(1),
    persistP50: pct(persist, 0.5),
    persistP95: pct(persist, 0.95),
    persistMax: Math.max(...persist),
    e2eP50: pct(deliver, 0.5),
    e2eP95: pct(deliver, 0.95),
    e2eMax: Math.max(...deliver),
    lost: 0,
    dupes: 0,
  };
  metrics.push(m);
...
await test('L2 — 1,000 messages: no loss, no duplicates, bounded latency', async () => {
  ...
  b.ws.on('message', (raw) => {
    const f = JSON.parse(raw);
    if (f.type === 'message') {
      if (seen.has(f.message.id)) dupes++;
      seen.add(f.message.id);
      arrival.set(f.message.id, Date.now());
    }
  });
  b.waiters.length = 0;
  b.frames.length = 0;
  ...
  eq(
    pg(`SELECT count(*) FROM chat_messages WHERE "clientMessageId" LIKE '${S}-l2-%'`),
    '1000',
    '1000 rows in Postgres',
  );
  const m = {
    test: 'L2 1000 msgs',
    wallMs: wall,
    throughput: +(1000 / (wall / 1000)).toFixed(1),
    persistP50: pct(persist, 0.5),
    persistP95: pct(persist, 0.95),
    persistP99: pct(persist, 0.99),
    persistMax: Math.max(...persist),
    e2eP50: pct(deliver, 0.5),
    e2eP95: pct(deliver, 0.95),
    e2eMax: Math.max(...deliver),
...
await test('L3 — 20 concurrent users in one conversation, 20 messages each (400 total)', async () => {
  ...
  for (const c of clients)
    c.ws.on('message', (raw) => {
      const f = JSON.parse(raw);
      if (f.type === 'message') counts.get(c.label).add(f.message.id);
    });
      ...
      clients.map(async (c, ci) => {
        ...
        for (let i = 0; i < 20; i++)
          out.push(
            await c.request(
              'message.send',
              { room, text: `l3-${ci}-${i}`, clientMessageId: `${S}-l3-${ci}-${i}` },
              40000,
            ),
          );
        return out;
  ...
  const m = {
    test: 'L3 20 users x 20 msgs',
    totalMessages: 400,
    fanOutDeliveries: 400 * 20,
    wallMs: wall,
    throughput: +(400 / (wall / 1000)).toFixed(1),
    persistP50: pct(persist, 0.5),
    persistP95: pct(persist, 0.95),
    persistMax: Math.max(...persist),
    lost: 0,
...
await test('L4 — rapid connect/disconnect churn (200 cycles)', async () => {
  ...
  const leaked = redis('--scan --count 5000')
    .split('\n')
    .filter((k) => k.startsWith('raven:chat:conn:')).length;
  const m = {
    test: 'L4 200 connect/disconnect',
    wallMs: wall,
    cyclesPerSec: +(200 / (wall / 1000)).toFixed(1),
    failures,
    leakedRedisConnKeys: leaked,
    apiRssMbBefore: rss0,
    apiRssMbAfter: rss1,
  };
  metrics.push(m);
  note(JSON.stringify(m));
  const ttls = redis('--scan --count 5000')
    .split('\n')
    .filter((k) => k.startsWith('raven:chat:conn:'))
    .map((k) => Number(redis(`ttl ${k}`)));
  note(`residual conn keys: ${leaked}; TTLs: ${JSON.stringify(ttls.slice(0, 5))} (presenceTtlSeconds=45)`);
  eq(
    ttls.filter((t) => t === -1),
    [],
    'every residual connection key still carries a TTL — bounded, not leaked',
  );
...
await test('L5 — 10 conversations in parallel, 10 messages each', async () => {
  ...
  const results = await Promise.all(
    Array.from({ length: 10 }, async (_, r) => {
      const room = await mkRoom(`${S}-l5-${r}`, ['alice', 'bob']);
      const [ag, bg] = [await grant('alice', room), await grant('bob', room)];
      const a = new Client(ag.token, `a${r}`),
        b = new Client(bg.token, `b${r}`);
      await a.connect();
      await b.connect();
      await a.request('room.join', { room });
      await b.request('room.join', { room });
      ...
      b.ws.on('message', (raw) => {
        const f = JSON.parse(raw);
        if (f.type === 'message') got.add(f.message.id);
      });
      const ids = [];
      for (let i = 0; i < 10; i++)
        ids.push(
          (await a.request('message.send', { room, text: `l5-${r}-${i}`, clientMessageId: `${S}-l5-${r}-${i}` }, 30000))
            .message.id,
        );
  ...
  const rss1 = await settledRss();
  eq(
    results.filter((r) => r.received !== 10),
    [],
    'every conversation delivered all 10',
  );
  eq(
    results.filter((r) => r.crossTalk > 0),
    [],
    'no cross-conversation leakage',
  );
  const m = {
    test: 'L5 10 conversations',
    wallMs: wall,
    conversations: 10,
    messages: 100,
    crossTalk: 0,
...
await test('post-load: Redis is bounded and Postgres is consistent', async () => {
  ...
  note(
    `redis keys after load: ${keys.length}, memory: ${redis('info memory')
      .split('\n')
      .find((l) => l.startsWith('used_memory_human'))
      .trim()}`,
  );
  const orphans = pg(
    `SELECT count(*) FROM chat_messages m LEFT JOIN chat_conversations c ON c.id=m."conversationId" WHERE c.id IS NULL`,
  );
  eq(orphans, '0', 'no orphaned messages');
  const dupIdem = pg(
    `SELECT count(*) FROM (SELECT "conversationId","senderId","clientMessageId" FROM chat_messages WHERE "clientMessageId" IS NOT NULL GROUP BY 1,2,3 HAVING count(*)>1) x`,
  );
