...
const conv = must(await http('/v1/chat/conversations', { method: 'POST', token: apiKey, body: {
  name: `p3-${S}`, members: [{ userId: 'alice' }, { userId: 'bob' }],
} }), 201, 'conv');
const room = conv.publicId;
const grant = async (u) => must(await http('/v1/chat/tokens', { method: 'POST', token: apiKey, body: { userId: u, conversations: [room] } }), 201, `mint ${u}`);
const aliceGrant = await grant('alice'), bobGrant = await grant('bob');

let alice = new Client(aliceGrant.token, 'alice'), bob = new Client(bobGrant.token, 'bob');
...
await test('Alice → Bob: delivered exactly once, correct sender/room/id/timestamp', async () => {
  ...
  eq(bob.frames.filter((x) => x.type === 'message' && x.message.id === aliceMsgId).length, 1, 'exactly one delivery to bob');
  eq(alice.frames.filter((x) => x.type === 'message' && x.message.id === aliceMsgId).length, 1, 'exactly one echo to alice');
...
await test('Bob → Alice: delivered exactly once', async () => {
  ...
  eq(alice.frames.filter((x) => x.type === 'message' && x.message.id === bobMsgId).length, 1, 'exactly one delivery to alice');
...
await test('both messages persisted with correct senders', async () => {
  ...
  eq(page.data.map((m) => [m.senderId, m.text]), [['bob', 'B→A'], ['alice', 'A→B']], 'newest-first, correct attribution');
});
...
await test('Alice offline → Bob sends → Alice reconnects and catches up via `after`', async () => {
  ...
  const before = must(await http(`/v1/chat/conversations/${room}/messages?limit=1`, { token: aliceGrant.token }), 200, 'cursor');
  ...
  const missed = [];
  for (let i = 0; i < 3; i++) {
    const ack = await bob.request('message.send', { room, text: `while-offline-${i}`, clientMessageId: `off-${i}-${S}` });
  ...
  const page = must(await http(`/v1/chat/conversations/${room}/messages?after=${encodeURIComponent(catchUpCursor)}`, { token: aliceGrant.token }), 200, 'catch-up');
  const ids = page.data.map((m) => m.id);
  for (const id of missed) ok(ids.includes(id), `missed message ${id} recoverable`);
  eq(page.data.map((m) => m.text), ['while-offline-2', 'while-offline-1', 'while-offline-0'], 'exactly the missed ones, newest-first');
