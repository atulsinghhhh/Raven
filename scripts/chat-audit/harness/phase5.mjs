...
let alice = new Client(ag.token, 'alice'), bob = new Client(bg.token, 'bob'), carol = new Client(cg.token, 'carol');
for (const [c, g] of [[alice, ag], [bob, bg], [carol, cg]]) { await c.connect(); await c.request('room.join', { room }); }
await sleep(250);

const historyCount = async () => (must(await http(`/v1/chat/conversations/${room}/messages?limit=100`, { token: apiKey }), 200, 'h')).data.length;
...
await test('Alice starts typing → Bob receives typing.started', async () => {
  ...
  eq(f.userId, 'alice', 'userId'); eq(f.roomId, room, 'roomId');
});
...
await test('rapid typing → one started event, not N', async () => {
  bob.clear();
  for (let i = 0; i < 15; i++) { alice.send({ type: 'typing.start', room }); await sleep(20); }
  await sleep(600);
...
await test('typing TTL expires on its own (no stop frame sent)', async () => {
  ...
  note('TTL-driven expiry works; note no typing.stopped event is broadcast on expiry — clients rely on their own local timer');
});
...
await test('multiple participants typing simultaneously', async () => {
  ...
  await sleep(500);
  const who = carol.frames.filter((f) => f.type === 'typing.started').map((f) => f.userId).sort();
...
await test('reconnect during typing clears that connection\'s typing state', async () => {
  ...
  await alice.connect(); await alice.request('room.join', { room });
...
await test('MULTI-SESSION typing: second session must not be cleared by the first closing', async () => {
  const a1 = new Client(ag.token, 'a1'); await a1.connect(); await a1.request('room.join', { room });
  const a2 = new Client(ag.token, 'a2'); await a2.connect(); await a2.request('room.join', { room });
  await sleep(200);
