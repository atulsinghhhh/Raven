...
const bob = new Client(bg.token, 'bob'); await bob.connect(); await bob.request('room.join', { room });
for (let trial = 1; trial <= 3; trial++) {
  ...
  const t0 = Date.now();
  a1.close();
  let offlineAt = null, onlineAt = null;
  while (Date.now() - t0 < 30000) {
    ...
    if (offlineAt !== null && !onlineAt && evs.some((e) => e.status === 'online')) { onlineAt = Date.now() - t0; break; }
    await sleep(100);
  }
  const p = await presence();
  console.log(`trial ${trial}: spurious offline at +${offlineAt}ms, recovered online at +${onlineAt ?? '>30000'}ms  (false-offline window ≈ ${onlineAt !== null ? onlineAt - offlineAt : '>30000'}ms) final=${JSON.stringify(p.map(x=>x.userId+':'+x.status))}`);
  a2.close();
  await sleep(1200);
}
bob.close(); process.exit(0);
