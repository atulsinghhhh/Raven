...
void main() {
  ...
  Matcher isRoomNotFound() =>
      isA<RavenException>().having((e) => e.code, 'code', RavenErrorCode.roomNotFound);

  group('Raven.join room/token precheck', () {
    test('accepts a roomId matching the token\'s room name (rnm), not only its internal id (rid)', () async {
      final token = fakeToken({'rid': 'room-internal-uuid', 'rnm': 'stream_public_id', 'sub': 'alice'});
      final raven = Raven(token: token, endpoint: unreachableEndpoint);

      await expectLater(raven.join('stream_public_id'), throwsA(isNot(isRoomNotFound())));
    });
    ...
    test('accepts a roomId matching the internal id (rid) too', () async {
      ...
      await expectLater(raven.join('room-internal-uuid'), throwsA(isNot(isRoomNotFound())));
    });
    ...
    test('rejects a roomId matching neither claim', () async {
      ...
      await expectLater(raven.join('totally-different-room'), throwsA(isRoomNotFound()));
    });

    test('skips the room-match check entirely for a token carrying neither claim', () async {
      ...
      await expectLater(raven.join('whatever-room'), throwsA(isNot(isRoomNotFound())));
    });

    test('a token with no rid cannot join at all, even if the caller-supplied roomId matches rnm', () async {
      ...
      await expectLater(
        ...
        throwsA(isA<RavenException>().having((e) => e.code, 'code', RavenErrorCode.invalidToken)),
      );
    });
  });
}
