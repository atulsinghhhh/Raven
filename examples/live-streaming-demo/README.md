...
reactions — built entirely on `@raven/client`'s `LiveStream` API
...
```bash
pnpm --filter @raven/rtc --filter @raven/chat --filter @raven/client run build
...
- A host publishing real camera/microphone through `LiveStream.join()` →
  ...
  `@raven/rtc` `Room` methods, not anything live-streaming-specific.
...
- Chat both ways over the stream's auto-attached conversation, using
  `@raven/chat` exactly as documented elsewhere — no second chat
  implementation.
