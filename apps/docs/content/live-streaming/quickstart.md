...
```bash
curl -X POST "$RAVEN_API_URL/v1/live-streams/stream_jRoD1T3EXh0PMJRGG4zYzQ/hosts" \
  -H "Authorization: Bearer $RAVEN_API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST "$RAVEN_API_URL/v1/live-streams/stream_jRoD1T3EXh0PMJRGG4zYzQ/start" \
  -H "Authorization: Bearer $RAVEN_API_KEY"
```
...
```bash
curl -X POST "$RAVEN_API_URL/v1/live-streams/stream_jRoD1T3EXh0PMJRGG4zYzQ/viewer-tokens" \
  -H "Authorization: Bearer $RAVEN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"identity": "carol"}'
```
...
```bash
curl -X POST "$RAVEN_API_URL/v1/live-streams/stream_jRoD1T3EXh0PMJRGG4zYzQ/end" \
  -H "Authorization: Bearer $RAVEN_API_KEY"
```
