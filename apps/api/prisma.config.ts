...
export default defineConfig({
  ...
  datasource: {
    ...
    // container) needs only DATABASE_URL. `||` rather than `??` so that an
    // empty DIRECT_URL — what a compose/k8s default of "" produces — falls
    // back instead of handing the schema engine a blank connection string.
    ...
    // there is nothing sensible to default it to — point it at a scratch
