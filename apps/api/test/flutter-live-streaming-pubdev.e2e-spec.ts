...
/**
 * Published-package validation twin of `flutter-live-streaming.e2e-spec.ts`.
 *
 * Everything below is identical to that file's suite — same backend, same
 * viewer harness, same assertions — with exactly one difference: the
 * Flutter host build comes from `flutter_check/published_consumer`, which
 * depends on `raven_live`/`raven_rtc` as plain hosted pub.dev version
 * constraints (see its `pubspec.yaml` — no `dependency_overrides`, no
 * `path:`). `flutter-live-streaming.e2e-spec.ts` proves the working tree
 * works; this file proves what an external developer actually gets from
 * `flutter pub add raven_live raven_rtc` today behaves the same way.
 *
 * Do not fold this into the original suite: the two builds must stay
 * provably distinct (local path vs. hosted), and a shared LIVE_HOST_WEB_DIR
 * would silently make that no longer true the next time someone edits one
 * and forgets the other.
 */
jest.setTimeout(180_000);

const LIVE_HOST_WEB_DIR = join(
  __dirname,
  '..',
  '..',
  '..',
...
describe('raven_live + raven_rtc (published pub.dev packages) — Flutter host to browser viewer (real E2E)', () => {
  ...
  beforeAll(async () => {
    if (!existsSync(join(LIVE_HOST_WEB_DIR, 'index.html'))) {
      throw new Error(
        `No Flutter web build at ${LIVE_HOST_WEB_DIR}.\n` +
          ...
          '-t lib/main_live_host.dart -o build/web_live_host)',
      );
    }
    ...
    const registered = await request(app.getHttpServer())
      ...
      .send({ email: `flutter-live-pubdev-e2e-${suffix}@raven.local`, password: 'correct-horse-battery-staple' })
      .expect(201);
    ...
    const project = await request(app.getHttpServer())
      ...
      .send({ name: `flutter-live-pubdev-e2e-${suffix}` })
      .expect(201);
  ...
  async function createStream(): Promise<string> {
    const res = await request(app.getHttpServer())
      ...
      .send({ title: `Flutter Live pub.dev E2E ${suffix}`, hostIdentity: 'alice' })
      .expect(201);
    return res.body.id;
  }
  ...
  it(
    'a Flutter host (published raven_live/raven_rtc) publishes camera+mic to a real live stream, and a real ' +
      'browser viewer receives actual increasing video frames/bytes, not just signaling',
    async () => {
      ...
      try {
        ...
        const hostLog = collectPageDiagnostics(hostPage, 'flutter-host-pubdev');
  ...
  it('a real browser host publishes to a real live stream, and a Flutter viewer (published raven_live/raven_rtc) subscribes to it', async () => {
    const streamId = await createStream();
    ...
    try {
      ...
      viewerCtx = await browser.newContext();
      const viewerPage: Page = await viewerCtx.newPage();
      const viewerLog = collectPageDiagnostics(viewerPage, 'flutter-viewer-pubdev');

      await viewerPage.goto(flutterAppUrl(viewerCreds));
      await waitForPage(
        viewerPage,
        () => (window as unknown as { __state?: { ready?: boolean } }).__state?.ready === true,
        viewerLog,
