...
/**
 * Separate from the RTC store's context on purpose. A page can render
 * `<RavenRoom>` and `<RavenChat>` side by side — a video call with a chat
 * panel — and neither provider should be able to satisfy the other's
 * hooks by accident (spec §45: independent SDKs that work together).
 */
