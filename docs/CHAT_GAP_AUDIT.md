...
| 4 | Public/private channels | ⚪ N/A BY DESIGN | No self-serve discovery/join is correct for the Agora Chat model — the developer's backend decides membership. Raven's `ROOM` type (host-controlled join via RTC/live-streaming) is the right analog for "anyone can join if given access," not a public directory |
...
|---|---|
| 1. Create a Raven project | Outside chat scope — handled by the platform's project/onboarding flow. |
