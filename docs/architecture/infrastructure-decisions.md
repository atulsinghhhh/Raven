...
This is the single source of truth for "what technology did we choose and
why" at the start of the project. Individual component docs
(`webrtc.md`, `sfu-comparison.md`, `turn.md`, `signaling.md`) go deeper on
specific areas. Revisit this document whenever a later phase reconsiders a
choice made here — do not let decisions drift silently.

## Decisions

| Component | Choice | Why |
|---|---|---|
| SFU + signaling | **LiveKit** (self-hosted, Apache-2.0) | Ships signaling, room/participant/token model, simulcast, and server SDKs together. Fastest path to a reliable end-to-end session without building our own signaling server. See `sfu-comparison.md`, `signaling.md`. |
| TURN/STUN | **coturn** | De facto open-source standard; engineering rule 3 mandates it; integrates as an external service alongside LiveKit. See `turn.md`. |
...
| Local development | **Docker Compose** | Engineering rule 6 and Rule 4 (local-first); every service (Postgres, Redis, coturn, LiveKit, API, dashboard) must run with `docker compose up -d`. |
