# @ravenkash/react — React hooks & optional UI primitives

React integration on top of `@ravenkash/rtc` — Phase 11's main deliverable.
Headless by default (hooks work with any UI you build), plus a handful
of genuinely optional components for a fast start. `@ravenkash/rtc` itself
...
```tsx
'use client';
import { RavenRoom, useConnectionState, useLocalParticipant, useRemoteParticipants, useCamera, ParticipantView } from '@ravenkash/react';
...
model `@ravenkash/rtc` itself uses (an RTC token is minted for exactly one
...
```tsx
import { RavenVideo, RavenAudio, ParticipantView, LocalParticipantView } from '@ravenkash/react';
...
`@ravenkash/react`. You still need it in **your own** component file if that
...
`@ravenkash/rtc` types you'll commonly need (`Room`, `Participant`, `Track`,
`RTCError`, `ConnectionState`, `DeviceInfo`, etc.) so most apps don't
need a direct `@ravenkash/rtc` import just for types. Media-plane types are
