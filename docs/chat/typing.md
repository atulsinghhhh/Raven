...
2. **Client-side timeout.** `@ravenkash/chat` arms a local timer on
   `startTyping()` and stops automatically after a pause, so a user who
   wanders off mid-sentence stops showing as typing.
3. **Receiver-side expiry.** `@ravenkash/react`'s store expires a typing user
...
```jsx
import { useTyping } from '@ravenkash/react';
