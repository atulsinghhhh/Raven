...
## "authentication required" from ACR is usually the wrong user

```
error from registry: authentication required, visit https://aka.ms/acr/authorization
```

This reads as a disabled admin user, an expired token or a missing image.
Check those first — they are one command each:

```bash
source infrastructure/azure/00-variables.sh
az acr show -n "$RAVEN_ACR" -g "$RAVEN_RG" --query adminUserEnabled     # expect true
az acr repository show-tags -n "$RAVEN_ACR" --repository raven-sfu -o tsv
```

If both look right, it is the third cause, and it is the common one:
`docker login` writes the credential to the **invoking user's**
`~/.docker/config.json`. `06-deploy-sfu.sh` runs as `ravenadmin`, so root
has no credential, and a pull as root fails while the identical pull as
`ravenadmin` succeeds:

```bash
# fails: authentication required
sudo docker compose --env-file /opt/raven/.env pull sfu
# succeeds
sudo -u ravenadmin docker compose --env-file /opt/raven/.env pull sfu
```

Anything running as root hits this: `sudo docker compose pull`, a systemd
unit, and `az vm run-command invoke` — which is exactly what you reach for
when you cannot SSH, because **`raven-sfu-nsg` allows port 22 from one
address only** (`AllowSshFromAdmin`, priority 300). A home or office IP
change locks you out of SSH and pushes you onto run-command, where every
pull runs as root and reports a registry error that has nothing to do with
the registry.

```bash
# check whether your current address is the allowlisted one
az network nsg rule show -g "$RAVEN_RG" --nsg-name "$RAVEN_SFU_NSG" \
  -n AllowSshFromAdmin --query sourceAddressPrefix -o tsv
curl -s https://api.ipify.org; echo
```

`06-deploy-sfu.sh` now logs in as both `ravenadmin` and root, so a pull
from either succeeds. If you hit this on a VM deployed before that change,
re-running the script is the fix.
